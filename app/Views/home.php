...
<body class="bg-slate-300">
	...
	<div class="container mx-auto px-2">
		<div
			class="mx-auto bg-slate-200 rounded-3xl max-w-md py-14 flex flex-col items-center justify-between gap-10 shadow-md">

			<img src="assets/img/vertical_lite_svg.svg" alt="Logo UniReport"
			...
			<form action="<?= url_to('home.insert') ?>" method="post" enctype="multipart/form-data" class="">
				<div class="grid gap-8">
					<div class="flex flex-col bg-white p-4 rounded-3xl shadow-xl">
						<label for="textarea" class="font-medium text-gray-500 pb-2 cursor-pointer">Descrição do
							problema</label>
						<textarea name="mensagem" id="textarea" cols="30" rows="1"
							class="outline-none px-2 focus:outline-2 focus:outline-blue-200 rounded-xl transition-all text-gray-500 cursor-pointer"
					...
					<div class="flex flex-col bg-white p-4 rounded-3xl shadow-xl">
						...
						<label for="fileInput"
							class="bg-sky-100 hover:bg-slate-200 text-blue-800 font-medium py-2 px-4 text-sm rounded-full cursor-pointer relative transition-all overflow-hidden max-w-md">
							<span class="block truncate w-64" id="fileLabel">Selecione o arquivo</span>
							<input type="file" name="arquivo_media" id="fileInput" class="hidden"
								accept=".pdf, .jpg, .png" onchange="updateFileName()">
						</label>
					</div>
				</div>

				<button type="submit"
					class="py-4 w-full bg-gradient-to-tr from-blue-950 to-blue-800 rounded-full text-white font-semibold mt-8 hover:bg-blue-900 transition-all shadow-xl hover:shadow-slate-500">Enviar</button>
			</form>
		</div>
	</div>
